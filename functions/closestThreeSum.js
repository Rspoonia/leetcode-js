var threeSumClosest = function (nums, target) {
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    let start = i+1,
      ...
      element = nums[i];
    while (start < end) { 
       if (Math.abs(element + nums[start] + nums[end] - target) < diff) {
        diff = Math.abs(element + nums[start] + nums[end] - target);
        res = element + nums[start] + nums[end];
      }  
